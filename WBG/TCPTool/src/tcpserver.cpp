...
void TcpServer::readyReadData()
{
    ...
    QString data=curClient->readAll();
    recContent.append(data);
    QByteArray start = "MsgType";
    QByteArray end = "MsgEnd";
    if (!recContent.isEmpty()){
        //qDebug() << recContent;
        while(recContent.contains(start) && recContent.contains(end)){
            if(recContent.startsWith(start) && recContent.contains(end)){
                ...
                //qDebug() << startIndex;
                //qDebug() << MsgLength;
//                qDebug()<<"msg:"<<MsgLength;
//                qDebug() <<"buffer:"<<recContent.size();
