...
class FtpControlConnection : public QObject
{
    Q_OBJECT
public:
    explicit FtpControlConnection(QObject *parent, QTcpSocket *socket, const QString &rootPath, const QString &userName = QString(), const QString &password = QString(), bool readOnly = false);
...
private:
    ...
    // Open a new passive data connection.被动模式，服务器开一个端口监听客户端的请求，客户端要
    // 上传和下载你就发命令来
    void pasv();
    ...
    //有关于数据的操作都被封装在这个类里，如list、上传、下载文件的时候
