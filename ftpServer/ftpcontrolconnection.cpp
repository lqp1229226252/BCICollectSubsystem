...
//每次对服务器中的操作(上传，下载，切换文件夹等)都会变成一条一条的命令发送给服务器，然后
//服务器会在这里解析，根据不同的命令来调用不同的函数
...
//    int port = exp.cap(2).toInt() * 256 + exp.cap(3).toInt();
    ...
    QRegularExpression regexp("\\s*(\\d+,\\d+,\\d+,\\d+),(\\d+),(\\d+)");
    int index = 0;
    while(index < addressAndPort.length())
    {
        auto match = regexp.match(addressAndPort, index);
        if(match.hasMatch())
        {
